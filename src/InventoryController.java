...
public class InventoryController {
    ...
    //method to request delete a laptop with given laptop id
    public void requestDeleteLaptopById(int laptopId) {

        //This message should arrive from the UI. Send a message to the db to request that this laptop is added.
        //Throws LaptopDataAccessException in case of unrecoverable errors e.g. database corrupted or programmer
        //error in writing database code or SQL statements.
        Laptop deletedLaptop = null;
        try {
            deletedLaptop = db.deleteLaptop(laptopId);//call delete laptop on model with laptop id
        }  catch (RuntimeException le) {
            System.out.println("Failed to delete laptop " + deletedLaptop);//if exception print message
            throw le;   //crash the program, programmer needs to fix this
        }

        if (deletedLaptop != null) {//if deletedLaptop is not null which means laptop deleted
            System.out.println("Laptop " + deletedLaptop + " deleted from database");//print laptop deleted
        } else {
            System.out.println("Laptop with Id = " + laptopId + " could not be deleted from database");//print no laptop deleted from database
        }
    }

    //method to request reassign a laptop with given laptop id
    public boolean requestReassignLaptop(int laptopId, String newUser) {
        boolean reassignSuccess = false;
        try {
            reassignSuccess = db.reassignLaptop(laptopId, newUser);//call reassign laptop on model
        }  catch (RuntimeException le) {
            throw le;   //crash the program, programmer needs to fix this
        }

        return reassignSuccess;//return true or false depending upon successful on reassigning the user
    }
